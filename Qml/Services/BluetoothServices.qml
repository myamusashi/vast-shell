...
Singleton {
    ...
    readonly property int connectedCount: BluetoothDeviceIndex.connectedCount(Bluetooth.devices.values, adapter)
    readonly property bool hasConnected: connectedCount > 0

    readonly property bool hasPaired: BluetoothDeviceIndex.hasWhere(Bluetooth.devices.values, adapter, "paired")

    readonly property bool hasBlocked: BluetoothDeviceIndex.hasWhere(Bluetooth.devices.values, adapter, "blocked")

    readonly property var pairedDevices: BluetoothDeviceIndex.pairedDevices(Bluetooth.devices.values, adapter)

    readonly property var availableDevices: BluetoothDeviceIndex.availableDevices(Bluetooth.devices.values, adapter)

    readonly property var blockedDevices: BluetoothDeviceIndex.blockedDevices(Bluetooth.devices.values, adapter)

    readonly property string headerSubtitle: BluetoothDeviceFormatter.headerSubtitle(adapterAvailable, adapterBlocked, adapter ? adapter.state : -1, adapterEnabled, isDiscovering)
