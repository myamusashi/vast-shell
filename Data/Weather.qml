...
Singleton {
    ...
    readonly property int tempMinData: forecast
                                       && forecast.length > 0 ? forecast[0].mintempC : 0
    readonly property int tempMaxData: forecast
                                       && forecast.length > 0 ? forecast[0].maxtempC : 0
    ...
    function reload() {
        ...
        sendRequest(url, function (response) {
            if (response.status === 200) {
                ...
                console.error("Weather request failed with status:",
                              response.status)
            }
        })
    }
