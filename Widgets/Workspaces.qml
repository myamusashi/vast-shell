...
RowLayout {
	...
	StyledRect {
		...
		implicitHeight: 25
		color: workspaceMBarArea.containsPress ? Themes.withAlpha(Themes.colors.surface_container_highest,
																  0.08) : workspaceMBarArea.containsMouse ? Themes.withAlpha(
					...
					GridLayout {
						...
						Repeater {
							...
							delegate: IconImage {
								...
								source: Quickshell.iconPath(DesktopEntries.heuristicLookup(waylandHandle?.appId)?.icon,
															"image-missing")
